...
class EditModel(object):
    ...
    def deleteEdits(self, word):
        # TODO: write this
        # Tip: If deleting the letter 'e' in the word 'test', the corrupt signal is 'te' and
        #      the correct signal is 't'.
        # Tip: If the length of the word is <= 0, return an empty list since no edits would be
        #      possible.
        ...
        for i in range(1, len(word)):
            ...
            correctLetters = word[i - 1]
            edits.append(Edit(corrupted[1:], corruptLetters, correctLetters))
    ...
    def insertEdits(self, word):
        ...
        for i in range(len(word)):
            corrupted = word[i]  # The corrupted signal is just this current character
            for alpha in self.ALPHABET:  # Iterate through the alphabet
                # The correct signal is the corruption + the insertion
                corrected = "%s%s" % (corrupted, alpha)
                correction = "%s%s%s" % (word[1:i + 1], alpha, word[i + 1:])  # The word post-insertion

                ret.append(Edit(correction, corrupted, corrected))
        return ret
    ...
    def transposeEdits(self, word):
        ...
        for i in range(len(word) - 1):
            transposed_word = word[:i] + word[i + 1] + word[i] + word[i + 2:]
            corruptLetters = word[i: i + 2]
            correctLetters = word[i + 1] + word[i]
            
            edits.append(Edit(transposed_word, corruptLetters, correctLetters))
        ...
        for i in range(len(word)):
            ...
            for a in self.ALPHABET:
                if a != corruptLetter:
                    ...
                    replaced_word = word[:i] + a + word[i + 1:]
                    
                    correctLetter = a
                    
                    edits.append(Edit(replaced_word, corruptLetter, correctLetter))
